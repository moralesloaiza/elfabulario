// Selección determinista de la "fábula de la semana".
//
...
// fecha y un pozo de fábulas elegibles, devuelve SIEMPRE la misma pieza durante
// los siete días de esa semana. Así todos los visitantes ven la misma fábula, y
// el cambio efectivo lo dispara el rebuild semanal (ver
// .github/workflows/fabula-semanal.yml, que hace ping al Deploy Hook de
// Cloudflare cada lunes).
//
// Propiedades de diseño:
//   - Determinista: mismo (pozo, fecha) → misma fábula. Sin Math.random.
//   - Barajado estable: el orden NO es cronológico (para que semanas seguidas no
//     caigan en fábulas vecinas), pero es fijo — depende solo del id.
//   - Cíclico: recorre todo el pozo antes de repetir. Con ~300 fábulas
//     ilustradas son ~6 años de turnos únicos.
//
...
/**
 * Devuelve la fábula que toca en la semana de `ahora`, o `null` si el pozo está
 * vacío. El índice se normaliza con módulo positivo para tolerar fechas
 * anteriores a la época.
 */
export function fabulaDeLaSemana<T extends { id: string }>(
	pozo: readonly T[],
	ahora: Date,
): T | null {
